...
async function connectMongoDB() {
    ...
    if (!mongoUri) {
        console.warn('MONGODB_URI not set — running with local file-based sessions only (no cross-restart backup).');
        return;
    }
    try {
        ...
        console.error('MongoDB connection failed, continuing with local file-based sessions only:', error.message);
    }
}
connectMongoDB();
...
async function autoReconnectOnStartup() {
    try {
        ...
        const sessions = isMongoConnected() ? await Session.find({}, 'number').lean() : [];
